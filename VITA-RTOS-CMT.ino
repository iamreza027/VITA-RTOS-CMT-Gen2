...
//SimpleTimer
SimpleTimer TimerAlarmShiftLever;
...
/* ============================================================
  ...
  =============================================================== */
typedef enum {
  AUDIO_COASTING_GEAR = 1,
  AUDIO_OVER_SPEED,
  AUDIO_INTER_AXLE_OVER_LIMIT,
  AUDIO_RELEASE_RETARDER,
  AUDIO_SUDAH_7_DETIK,
  AUDIO_ENGINE_OVER_RUNNING,
  AUDIO_GEAR_DI_TANJAKAN,
  AUDIO_CAN_BUS_LOSE,
  AUDIO_SD_CARD_LOSE,
  AUDIO_VITA_ERROR,
  AUDIO_LIMIT_MIS_OPERATION,
  AUDIO_OVER_LIMIT_MIS_OPERATION,
  AUDIO_LOW_POWER,
  AUDIO_SERVICE_BRAKE_OVER_LIMIT,
  AUDIO_NORMAL_VITA,
  AUDIO_PROXIMITY_TIDAK_NORMAL,
  AUDIO_RFID_DITERIMA,
  AUDIO_RFID_DITOLAK,
  AUDIO_SPEED_LIMIT,
  AUDIO_HUNTING_V1,
  ...
  AUDIO_ALARM_3_KALI,
  AUDIO_ALARM_4_KALI,
  AUDIO_MODE_TRAVEL_AKTIF,
  AUDIO_MODE_TRAVEL_MATI,
  AUDIO_ISTIRAHAT_TRAVEL_JAUH,
  AUDIO_PRAEVENT_MODE_TRAVEL,
  AUDIO_EVENT_TRAVEL_JAUH,
  AUDIO_NOTICE_LONG_TRAVEL,
  AUDIO_MODE_TRAVEL_SELESAI,
  AUDIO_INCLINO_LESS_10,
  AUDIO_INCLINO_LESS_15,
  AUDIO_INCLINO_MORE_10,
  AUDIO_INCLINO_MORE_15,
  AUDIO_INCLINOMETER_LOSE,
  AUDIO_EVENT_V13_KODE_4,
  AUDIO_DIFFLOCK_OVER_LIMIT,
  AUDIO_ELECTRIC_BRAKE_OVERLIMIT,
  AUDIO_WARNING_7_DETIK,
  AUDIO_WARNING_5_DETIK,
  AUDIO_WARNING_3_DETIK,
  AUDIO_WARNING_10_METER,
  AUDIO_REVERSE_OVER_DISTANCE,
  AUDIO_WARNING_IDLE,
  AUDIO_EVENT_IDLE_OVERLIMIT,
  AUDIO_RESET_VAD_IDLE,
  AUDIO_CRITICAL_SOC,
  AUDIO_WARNING_15_METER

} AudioEvent;

void fillEventTime(EventItem *item) {  //Function pengambilan waktu untuk fill event
...
void parseOverspeedLimit() {
  OverspeedMuatanON = atoi(deviceConfig.overspeedLimit[0]);
  OverspeedMuatanOFF = atoi(deviceConfig.overspeedLimit[1]);
  OverspeedKosonganON = atoi(deviceConfig.overspeedLimit[2]);
  OverspeedKosonganOFF = atoi(deviceConfig.overspeedLimit[3]);

  Serial.println("OVERSPEED CONFIG:");
  Serial.printf("Muatan ON  : %d\n", OverspeedMuatanON);
    ...
    if (!digitalRead(CAN_INT)) {
      if (xSemaphoreTake(spiMutex, pdMS_TO_TICKS(5)) == pdTRUE) {
        ...
        switch (canId) {
          ...
          default:
            // optional: ignore unknown CAN ID
            break;
        }
...
void taskCANLogic(void *pv) {
  for (;;) {
    ...
    bool TMabuse = abuseWarningFlag;
    requestAudio(AUDIO_ABUSE, TMabuse);
    if (abuseWarningFlag) {
      if (!abuseActive) {
        Serial.println("[EVENT] ABUSE TERDETEKSI (D <-> R)");

        abuseActive = true;
        abuseWarningFlag = false;
        playAudio(AUDIO_ABUSE);
    ...
    if (shiftReadyFlag) {
      requestAudio(AUDIO_SHIFTLEVER, true);  // aktif sebentar
      playAudio(AUDIO_SHIFTLEVER);
      Serial.println("[EVENT] SHIFT READY");
      shiftReadyFlag = false;
      requestAudio(AUDIO_SHIFTLEVER, false);
    }
...
void setup() {
  ...
  // Inisialisasi history awal dengan netral untuk logic V4
  unsigned long now = millis();
  currentGearRaw = CAN_getCurrentGear();
  int g0 = normalizeGear(currentGearRaw);
  addGearToHistory(g0, now);
  lastGearNorm = g0;
